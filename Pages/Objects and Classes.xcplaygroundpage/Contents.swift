...
class SurfBoard {
    ...
    var volume: Float = 0
    let tail: String = "Rocker"
    
    init(length: Double, width: Int, height: Int) {
        self.length = length
        self.width = width
        self.height = height
    }
    ...
    func boardDescription() -> String {
        let newVolume = calculateVolume()
        
        return "Your brand new board has: \(length), \(width), \(height), \(newVolume) and \(tail)"
    }
    
    func calculateVolume() -> Float {
        let calculation =  width * height * Int(length)
        
        volume = Float(calculation / 1000)
        return volume
    }
}

var newBoard = SurfBoard(length: 1.80, width: 60, height: 170)
print(newBoard.boardDescription())
...
class Circle: NamedShape {
    ...
    override func simpleDescription() -> String {
        return "A circle has \(area()) of area"
    }
}

let newCircle = Circle(radius: 1.10, name: "Rodrigo")
newCircle.area()
newCircle.simpleDescription()
